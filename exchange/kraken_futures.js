...
module.exports = class Kraken {
    ...
    */
    ...
    ws.onopen = function() {
      ...
      symbols.forEach(symbol => {
        ...
        ws.send(JSON.stringify({ event: 'subscribe', feed:'ticker', product_ids: [`${symbol.symbol}`] }));
        if (config.getOrderbook != undefined && config.getOrderbook == true) {
          //console.log('Kraken: Subscribing orderbook', symbol.symbol);
      ...
      if (true == false /*skip, not implemented right now!*/ && config.key && config.secret && config.key.length > 0 && config.secret.length > 0) {
        me.logger.info('Kraken: sending auth request');
        ...
        me.logger.info('Kraken: Starting as anonymous; no trading possible');
      }
    };
    ...
    ws.onmessage = async function(event) {
      if (event.type === 'message') {
        ...
        if ('success' in data && data.success === false) {
          me.logger.error(`Kraken: error ${event.data}`);
          console.log(`Kraken: error ${event.data}`);
        } else if (data.event && data.message && data.event === 'error' && data.message == 'invalid json') {
        ...
        } else if (data.feed && data.feed == 'ticker') {
          let bid = data.bid;
          let ask = data.ask;

          let symbol = data.product_id;
  ...
  _ping(ws) {
    ...
    ws.send(JSON.stringify({event:'unsubscribe', feed:'abc'})); // force error to get ping response
  ...
  async order(order, skipLeverage) {
    console.log('*** Kraken, creating order', order);
