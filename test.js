...
var params = {screen_name: accounts[i], count: 100};
	  ...
	  for (var t in tweets){
		  ...
		  for (var s in split){
			  if (symbols.includes(split[s].toUpperCase().replace('$','').replace('#',''))){
				  ...
				  if (score > 5 && !tids.includes(tweets[t].id)){
					  tids.push(tweets[t].id)
					  if (new Date().getTime() - ts > 1000 * 60 * 60){
					  winners.push('score': score, 'ts': ts, 'coin': split[s].toUpperCase().replace('$','').replace('#','')})
					  console.log(winners)
					  }
				 }
			  }
